"""Weather AI Lambda - real weather lookup + Bedrock AI insight.
...
Bedrock (Claude 3.5 Haiku) for a short human-readable insight, and caches the
combined result in DynamoDB for CACHE_TTL_SECONDS.

Not yet implemented: the GCP Vertex AI failover path if Bedrock is
unavailable (planned - see weather-ai-app-account-structure.md "Next steps").
get_ai_insight() is the seam where that retry will go; right now a Bedrock
failure just surfaces as a 502.
"""
...
def get_ai_insight(conditions, city_label):
    ...
    try:
        ...
        raise UpstreamError(f"Bedrock invoke failed: {e}") from e
